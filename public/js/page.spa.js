...
$(window).scroll(function(e){
 ...
 $(".page").eq(now).find(".spa_ani").each(function(){//도착한 현재페이지 안에있는
    var name = "opaShow"
    var duration = "0.5s"
    var delay = 0;
    if($(this).data("name") != "" && $(this).data("name") !== undefined){
        name = $(this).data("name");  //html에서 data-name값
    }
    if($(this).data("duration") != "" && $(this).data("duration") !== undefined){
        duration = $(this).data("duration"); //html에서 data-duration값
    }
...
function barMove(obj){
   if(obj.width() == 0)  obj.stop().animate({"width":obj.html()}, 500);
}